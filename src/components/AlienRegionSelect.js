...
class AlienRegionSelect extends Component {
  ...
  isInExistedRegion(event) {
    return event.target.dataset.wrapper || event.target.dataset.dir
  }
  ...
  render() {
    return (
      <div
        ...
        onMouseUp={(event) => this.props.setRegionStateUnchangedDispatch()}
        onMouseMove={(event) => {
          event.preventDefault()
      ...
      >
        ...
        {this.props.regions.map((region, index) => (
          <ConnectedAlienRegion
            ...
            getClientPosInImage={(event) =>
              this.getClientPosInImage(event, this.refs.image)
            }
