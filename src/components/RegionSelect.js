...
class RegionSelect extends Component {
  ...
  onRegionMoveStart(event, index) {
    ...
    const regionWidth =
      (currentRegion.width / 100) * this.refs.image.offsetWidth
    const regionHeight =
    ...
    let regionChangeData = {
      ...
      imageWidth: this.refs.image.offsetWidth,
      imageHeight: this.refs.image.offsetHeight,
      changeState: resizeDir
        ? RegionChangeState.Resize
        : RegionChangeState.Move,
      resizeDir: resizeDir,
    }
  ...
  onDocMouseTouchMove(event) {
    ...
    switch (regionChangeData.changeState) {
      case RegionChangeState.Resize:
        region = this.updateRegionResize(clientPos, regionChangeData)
        break
    ...
    const rect = Object.assign({}, region, { isChanging: true })
    this.props.onChange([
      ...this.props.regions.slice(0, index),
  ...
  updateRegionResize(clientPos, regionChangeData) {
    let oppositeCornerX =
      ((regionChangeData.oppositeCorner.x - regionChangeData.imageOffsetLeft) /
        regionChangeData.imageWidth) *
      100
    let oppositeCornerY =
      ((regionChangeData.oppositeCorner.y - regionChangeData.imageOffsetTop) /
  ...
  updateRegionMove(clientPos, regionChangeData, updatingRegion) {
    let x =
      ((clientPos.x +
